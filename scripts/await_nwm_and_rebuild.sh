...
LOG_DIR="logs"
PID_FILES=(
  "$LOG_DIR/nwm_03486000_v2_retrospective_2010_2020_rerun.pid"
  "$LOG_DIR/nwm_03161000_v2_retrospective_2010_2020_rerun.pid"
)
...
for pidfile in "${PID_FILES[@]}"; do
  if [[ -f "$pidfile" ]]; then
    ...
    if kill -0 "$pid" 2>/dev/null; then
      echo "Waiting for $pidfile ($pid) to finish..."
